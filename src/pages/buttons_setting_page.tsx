...
import {
  disableFlipType,
  alwaysFlipType,
  flipIfPressedSelfType,
  flipIfPressedSomeButtonsType,
  ignoreButtonsInFlipingType,
  remapType,
  closeMenuType,
  applyMacroType,
  installMacroType,
...
export const ButtonsSettingPage = () => {
  ...
  const { layers, layersDispatch, prefixKeys, setPrefixKeys } = useContext(
  ...
  const [initializedSetting, setInitializedSetting] = useState(
    {} as ButtonsSettingType
  );
  ...
  const exportSetting = () => {
    const body = ButtonsSettingConverter({
        ...
        if (body.installed_modes) {
          body.installed_modes.forEach((modeName: string) => {
            layersDispatch({
              ...
              payload: { installed_mode: modeName },
            });
          });
        }

        setPrefixKeys(
          response.data.setting.prefix_keys_for_changing_layer.sort()
        );
        ...
        layerKeys.forEach((layerKey) => {
          ...
          if (modeTable) {
            Object.entries(modeTable).forEach((val, i) => {
              ...
              layersDispatch({
                type: applyModeType,
                payload: { layerKey: layerKey, mode: mode },
              });
            });
          } else {
            layersDispatch({
              type: applyModeType,
              payload: { layerKey: layerKey, mode: { name: "disable" } },
            });
          }
          ...
          buttons.forEach((button) => {
            const buttonState = new ButtonState(
              button,
              layers[layerKey][button]?.flip,
              layers[layerKey][button]?.remap
            );
            ...
            if (layers[layerKey][button] === undefined) {
              ...
              layersDispatch({
                ...
                payload: {
                  layerKey: layerKey,
                  button: button,
                  targetButtons: layers[layerKey][button].remap?.to,
                },
              });
            } else if (buttonState.hasFlipSetting()) {
              layersDispatch({
                type: ignoreButtonsInFlipingType,
                payload: {
                  layerKey: layerKey,
                  button: button,
                  targetButtons: flip.force_neutral,
                },
              });

              if (buttonState.isDisabledFlip()) {
                layersDispatch({
                  type: disableFlipType,
                  payload: { layerKey: layerKey, button: button },
                });
                ...
                layersDispatch({
                  ...
                  payload: {
                    layerKey: layerKey,
                    button: button,
                    targetButtons: layers[layerKey][button].flip?.if_pressed,
                  },
                });
              }
            } else {
    ...
    <>
      ...
      >
        ...
        >
          <h2></h2>
          <div>
            <a href="#" onClick={applySetting}>
              変更した設定でsetting.ymlへ上書きする
            </a>
            <div>{infoMessage}</div>
            <ul>
              {changes().map((c, i) => (
      ...
      {layerKeys.map((l, index) => (
        <ButtonsSetting key={index} layerKey={l} layerRef={layerRefs[index]} />
      ))}
    </>
  );
};
