...
export const ButtonStateDiff = ({ before, after }: Props): Array<string> => {
  ...
  if (!before || !after || !before.layers) {
    return changes;
  }

  if (before.layers.installed_macros && after.layers.installed_macros) {
    const installedMacrosDiffResult = diff(
      before.layers.installed_macros,
      after.layers.installed_macros
    );
  ...
  if (before.layers.installed_modes && after.layers.installed_modes) {
    const installedModesDiffResult = diff(
      before.layers.installed_modes,
      after.layers.installed_modes
    );
  ...
  if (
    before.prefix_keys_for_changing_layer &&
    after.prefix_keys_for_changing_layer
  ) {
    if (
      before.prefix_keys_for_changing_layer.toString() ===
      after.prefix_keys_for_changing_layer.toString()
    ) {
      // no-op
    } else {
      changes.push(
        `key prefixは ${before.prefix_keys_for_changing_layer} => ${after.prefix_keys_for_changing_layer} になります`
      );
    }
  }
  ...
  layerKeys.forEach((layerKey) => {
    ...
    buttons.forEach((b) => {
      ...
      const diffResult = diff(
        before.layers[layerKey as LayerKey][b],
        after.layers[layerKey as LayerKey][b]
      );
