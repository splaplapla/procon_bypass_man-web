...
export const ButtonsSettingConverter = ({ prefixKeys, layers }: Props) => {
  ...
  if (!layers.installed_macros) {
    layers.installed_macros = {};
  }
  if (!layers.installed_modes) {
    layers.installed_modes = {};
  }
  if (!layers.up.macro) {
    layers.up.macro = {};
  }
  ...
  const installedMacroNames = Object.entries(layers.installed_macros)
    .filter((k, i) => k[1])
    .map((v) => v[0]);
  const installedModeNames = Object.entries(layers.installed_modes)
    .filter((k, i) => k[1])
    .map((v) => v[0]);
...
${installedMacroNames
  ...
  .join("\n")}
${installedModeNames
  .map((name) => `${topLevelIndent}install_mode_plugin ${name}`)
  .join("\n")}
